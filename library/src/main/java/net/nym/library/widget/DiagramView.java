...
public class DiagramView extends SurfaceView implements Callback {
    ...
    private long counts = 400;//UploadMonitor.TOTAL_TIME / UploadMonitor.PERIOD;
    private float xStep ;
    ...
    private void init() {
        ...
        mWidth = getWidth();
        mHeight = getHeight();
        xStep = (mWidth + 0.0f) / counts;
//		Log.e("xstep", "xstep=" + xStep);
    ...
    private void setGridData() {
        ...
        for (int i = 0; i < vLineCount; i++) {
            addLine(new Float[] { Float.valueOf((mWidth * 1f / vLineCount) * i),
                    Float.valueOf(0),
                    Float.valueOf((mWidth* 1f / vLineCount) * i),
        ...
        for (int i = 0; i < hLineCount; i++) {
            addLine(new Float[] { Float.valueOf(0),
                    Float.valueOf((mHeight* 1f / hLineCount) * i),
...
//				Float.valueOf(mHeight) };
        Float[] values = { Float.valueOf(data.size() / 2) * xStep,
                Float.valueOf(mHeight - value) };
...
//			}
            for (int j = 1; j < data.size(); j += 2) {
                Float f = data.remove(j);
                data.add(j , Float.valueOf(f - mHeight + height));
            }
        }
        ...
        mHeight = height;
        xStep = (mWidth + 0.0f) / counts;
//		if(mWidth != xStep * counts)
//		{
//			LayoutParams params = new LayoutParams((int) (xStep * counts),mHeight);
//		        setLayoutParams(params);
//		}
//		Log.e("xstep", "xstep=" + xStep + ",width=" + mWidth + ",count=" + counts);
        setGridData();
        isRun = true;
    }
